...
class Nakama {
  ...
  async findMatch() {
    try {
      ...
      async updateLikeness(playerNickname, npcNickname, value) {
        try {
            const payload = {
                "player_nickname": playerNickname,
                "npc_nickname": npcNickname,
                "value": value
            };
            const response = await this.client.rpc(this.session, "tx-update-likeness", payload);
            console.log("Likeness updated successfully", response);
        } catch (error) {
            console.error("Error updating likeness:", error.message);
        }
    }
